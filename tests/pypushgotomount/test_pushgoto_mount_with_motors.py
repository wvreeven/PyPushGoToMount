...
class TestPushGoToMount(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_pushgoto_mount(self) -> None:
        ...
        with mock.patch("pypushgotomount.controller.utils.CONFIG_FILE", config_file):
            async with pypushgotomount.PushGoToMount(run_forever=False) as self.pushgoto_mount:
                self.reader, self.writer = await asyncio.open_connection(host="localhost", port=11880)
                await self.send_start_commands()

                self.motor_controller_alt = (
                    self.pushgoto_mount.responder.mount_controller.motor_controller_alt
                )
                self.motor_controller_az = self.pushgoto_mount.responder.mount_controller.motor_controller_az
                ...
                await self.pushgoto_mount.stop()
    ...
    async def send_start_commands(self) -> None:
        ...
        for st, expected in [
            (f":{pypushgotomount.CommandName.St}+40*30#", "1"),
            (f":{pypushgotomount.CommandName.S_LOWER_G}003*53#", "1"),
            (f":{pypushgotomount.CommandName.S_UPPER_G}{-utcoffset_hours:04.1f}#", "1"),
            (f":{pypushgotomount.CommandName.SL}{now_time}#", "1"),
            (f":{pypushgotomount.CommandName.SC}{now_date}#", "1"),
            (f":{pypushgotomount.CommandName.U}#", None),
            (f":{pypushgotomount.CommandName.RG}#", None),
        ]:
            ...
            if expected is not None:
                ...
                if st.startswith(f":{pypushgotomount.CommandName.SC}"):
    ...
    async def set_and_assert_position(self, position: SkyCoord) -> None:
        ...
        altaz = await pypushgotomount.my_math.get_altaz_from_radec(position, self.now.timestamp())
        ...
        dec_str = position.dec.to_string(u.deg, precision=0, pad=True, alwayssign=True, sep=("*", ":"))
        self.writer.write(f":{pypushgotomount.CommandName.Sr}{ra_str}#".encode())
        await self.writer.drain()
        self.writer.write(f":{pypushgotomount.CommandName.Sd}{dec_str}#".encode())
        await self.writer.drain()
        self.writer.write(f":{pypushgotomount.CommandName.CM}#".encode())
        await self.writer.drain()
        await self.reader.readuntil(b"#")

    async def assert_position(self, expected: SkyCoord) -> None:
        self.pushgoto_mount.responder.mount_controller.position_event.clear()
        await self.pushgoto_mount.responder.mount_controller.position_event.wait()

        self.writer.write(f":{pypushgotomount.CommandName.GR}#".encode())
        await self.writer.drain()
        ra = (await self.reader.readuntil(b"#")).decode().strip("#")

        self.writer.write(f":{pypushgotomount.CommandName.GD}#".encode())
        await self.writer.drain()
